...
test('first run confirms and stores a full hostname without notifying', async () => {
  ...
  assert.equal(state.currentDomain, 'test-alias.cn59.ug.link');
...
test('a confirmed change commits a full hostname and sends a generic event', async () => {
  const config = await testConfig();
  const domains = [
    'cn59.ug.link', 'test-alias.cn59.ug.link',
    'cn60.ug.link', 'test-alias.cn60.ug.link'
  ];
  const delivered = [];
  const monitor = new Monitor(config, {
    logger: silentLogger(),
  ...
  assert.equal((await loadState(config, silentLogger())).state.currentDomain, 'test-alias.cn60.ug.link');
  assert.equal(delivered.length, 1);
  assert.equal(delivered[0].event_type, 'relay_changed');
  assert.equal(delivered[0].old_url, 'https://test-alias.cn59.ug.link');
  assert.equal(delivered[0].new_url, 'https://test-alias.cn60.ug.link');
...
test('different raw forms normalize to the same candidate during confirmation', async () => {
  const config = await testConfig();
  const domains = ['cn59.ug.link', 'test-alias.cn59.ug.link'];
  const monitor = new Monitor(config, {
    logger: silentLogger(),
    fetchRelayDomain: async () => domains.shift(),
    notificationDriver: captureDriver([]),
